---
name: init
description: 首次配置或重新配置 Pensieve。检查 pointer、可选启用项目级 kernel、在 CLAUDE.md 写入 Pensieve 配置段(Sub-Agent Context Block 模板 + 常用命令)。用户说 /init、"配置 Pensieve""初始化 Pensieve""Pensieve 怎么用"时触发
...
> 由 `/init` 生成。重新配置运行 `/init`,手动编辑直接改下方值。
...
- **审查提醒节奏**: daily: on / weekly: off / monthly: on
  - 关 daily:运行 `/retrospect --daily` 记一笔后当天不再提醒
...
- `/learn <描述>` — 记录一次踩坑/经验(也可扫当前对话找候选)
- `/promote <memory-path>` — 把已验证 2 次以上的 memory 升级为原则
- `/retrospect --daily | --monthly` — 日度笔记 / 月度审查清理
...
- 下一步建议:"试着运行 `/learn <你最近踩的一个坑>` 记录第一条经验"
...
>
> 这个 plugin 干同样的事:踩过的坑用 `/learn` 记录到 memory,反复出现的模式用 `/promote` 升级为原则 kernel,SessionStart 把原则自动注入每个 Claude session 作为行为约束。`/retrospect` 定期审查清理。
>
